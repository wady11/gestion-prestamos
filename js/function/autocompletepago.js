...
let hiddenMontoAnterior = document.getElementById("montopagoanterior");
let hiddenInteres = document.getElementById("hiddeninteres");
let hiddenValorPago = document.getElementById("valorrealpago");
...
window.addEventListener("load", () => {
            ...
            for (let incr = 0; incr < arr.length; incr++) {
                ...
                if (arr[incr].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
                    ...
                    createDivChild.addEventListener("click", function (e) {
                        ...
                        hiddenMontoAnterior.value = mostrarMonto.value;
                        hiddenValorPago.value = valorPago.value;
                        nameHidden.value = nombreField.value;

                        console.log(hiddenMontoAnterior.value);
                        //funcion
                        closeAllLists(inp);
                    });
    ...
    function MontoTrue(valor) {
        if (valor) {
            ...
            Swal.fire("ooops", `este cliente no tiene deudas!`, "error");
        }
    }
